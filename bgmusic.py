...
# Backgrouund music for EmulationStation 1.1
# Based on Livewire script at retropie.org.uk
...
# if you don't have pygame: sudo apt-get install python-pygame
...
# Only for pygame 1.5.9
environ['PYGAME_HIDE_SUPPORT_PROMPT'] = '1'

# LOCAL CONFIG

# Find everything that's .mp3 or .ogg
...
#Fill in all of the current RetroPie Emulator process names in this list.
...
while not esStarted:
	...
	for pid in pids:
		try:
			...
			if procname[:-1] == "emulationstation" or procname[:-1] == "emulationstatio": 
				# Emulation Station's actual process name is apparently short 1 letter.
				esStarted=True
		except IOError:	
			continue
...
if not startsong == "":
	try:
		# Set the currentsong to the index in BGM that is our startingsong.
		currentsong = bgm.index(startsong)
	except:
		# If this triggers, you probably screwed up the filename, because our startsong wasn't found in the list.
		currentsong = -1 

# This is where the magic happens.
while True:
	while not esStarted: 
		# New check (4/23/16) - Make sure EmulationStation is actually started.  There is code further down that, as part of the emulator loop, makes sure eS is running.
	...
	# If we aren't currently playing any music
	...
	for pid in pids:
		try:
			...
			if procname[:-1] in emulatornames: #If the process name is in our list of known emulators
				...
				# Ensures that the volume is manually set (if restart is True, volume would be at zero)
				volume=maxvolume

		# proc has already terminated, ignore.
		except IOError: 
			continue
...
# Theoretically you should never get this far.
